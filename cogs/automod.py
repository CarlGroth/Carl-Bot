...
class Automod:
    ...
    async def on_member_join(self, member):
        ...
        if log_channel == [] or log_channel[0][0] is None:
            ...
            log_channel = member.guild.get_channel(int(log_channel[0][0]))
        try:
            ...
            if re.match(r"^(?:.*\.(mp4|webm|mkv|mov)$)$", filename):
                ...
                if msg:
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.get(url="https://api.streamable.com/import?url={}".format(s_url), auth=aiohttp.BasicAuth(login="carl.groth@protonmail.com", password="google123")) as r:
                            reply = await r.json()
                    ...
                    while not embed:
                        ...
                        if (tries % 5) == 0:
                            async with aiohttp.ClientSession() as session:
                                async with session.get("https://api.streamable.com/videos/{}".format(reply['shortcode'])) as r:
                                    ree = await r.json()
                            if ree['status'] == 2:
                                embed = True
                            elif tries == 100:
