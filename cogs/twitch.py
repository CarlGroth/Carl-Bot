...
class Twitch:
            ...
            try:
                ...
                for row in a:
                    ...
                    if not data:
                        ...
                        log_channel = self.c.execute('SELECT id, twitch_channel FROM servers WHERE id=?', (row[0],))
    ...
    async def twitch(self, ctx, stream : str, *, name : str = None):
        ax = self.c.execute('SELECT * FROM servers WHERE id=?', (ctx.guild.id,))
        ax = ax.fetchone()
        if ax[2] is None:
        ...
        async with session.get(url, headers=header) as r:
            ...
            a = self.c.execute('SELECT * FROM twitch WHERE (id=? AND server=?)', (t_id, ctx.guild.id))
            a = a.fetchall()
            if a == []:
                self.c.execute("INSERT INTO twitch VALUES (?, ?, ?, ?, ?)", (ctx.guild.id, stream, t_id, name, 1479973395.693614))
                self.conn.commit()
                return await ctx.send('Twitch stream "{}" added with the alias "{}"'.format(stream, name))
            elif possible_deletion:
                self.c.execute('DELETE FROM twitch WHERE (id=? AND server=?)', (t_id, ctx.guild.id))
                self.conn.commit()
                return await ctx.send('Twitch stream "{}" deleted'.format(stream))
            else:
                self.c.execute('UPDATE twitch SET alias=? WHERE (id=? AND server=?)', (name, t_id, ctx.guild.id))
                self.conn.commit()
    ...
    async def twitch_all(self, ctx):
        a = self.c.execute('SELECT * FROM twitch WHERE server=?', (ctx.guild.id,))
        a = a.fetchall()
        if a != []:
            e = discord.Embed()
            name = '\n'.join([x[1] for x in a])
            alias = '\n'.join([x[3] for x in a])
            last_online = '\n'.join([human_timedelta(datetime.datetime.fromtimestamp(x[4])) for x in a])
