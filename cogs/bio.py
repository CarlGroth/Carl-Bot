...
class Bio:
    ...
    async def bio(self, ctx, *, member : discord.Member = None):
        ...
        bio = self.c.execute('SELECT content FROM bio WHERE (server=? AND owner=?)', (ctx.guild.id, owner.id))
        bio = bio.fetchall()
        if bio == []:
            ...
            bio = bio[0][0]
            print(bio)
            self.c.execute('UPDATE bio SET uses = uses + 1 WHERE (server=? AND owner=?)', (ctx.guild.id, owner.id))
            self.conn.commit()
    ...
    async def _append(self, ctx, *, content : str):
        ...
        a = self.c.execute('SELECT content FROM bio WHERE (owner=? AND server=?)', (lookup, ctx.guild.id))
        a = a.fetchall()
        if a == []:
            return await ctx.send("Can't append to bios that don't exist.")

        bio = a[0][0] + "\n{}".format(content)
        self.c.execute('UPDATE bio SET content=?, updated=? WHERE (owner=? AND server=?)', (bio, datetime.datetime.utcnow().timestamp(), lookup, ctx.guild.id))
        self.conn.commit()
    ...
    async def create(self, ctx, *, content : str):
        content = self.clean_bio_content(content)
        lookup = ctx.message.author.id
        a = self.c.execute('SELECT * FROM bio WHERE (owner=? AND server=?)', (lookup, ctx.guild.id))
        a = a.fetchall()
        if a != []:
            ...
            if msg.content.lower() in ['r', 'y', 'yes', 'replace']:
                self.c.execute('UPDATE bio SET content=? WHERE (owner=? AND server=?)', (content, lookup, ctx.guild.id))
                self.conn.commit()
                ...
                bio = self.c.execute('SELECT content FROM bio WHERE (server=? AND owner=?)', (ctx.guild.id, lookup))
                bio = bio.fetchall()
                bio = bio[0][0]
                bio = bio + "\n{}".format(content)
                self.c.execute('UPDATE bio SET content=?, updated=? WHERE (owner=? AND server=?)', (bio, datetime.datetime.utcnow().timestamp(), lookup, ctx.guild.id))
        ...
        print(a)
        self.c.execute('INSERT INTO bio VALUES (?, ?, ?, ?, ?, ?)', (lookup, content, ctx.guild.id, datetime.datetime.utcnow().timestamp(), datetime.datetime.utcnow().timestamp(), 0))
        self.conn.commit()
